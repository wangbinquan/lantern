...
- **只读侵入式诊断 `lantern observe`**(RFC-0004):对 JVM 用 **Arthas 批处理**(watch/trace/stack/tt)看方法**入参/返回/调用路径/耗时**——**只读 by construction**(固定表达式、无任意 OGNL、动词白名单)、不开端口、需确认、watch 可见。区别于 `snapshot` 的被动一击。
...
🟢 **MVP 已实现**(TypeScript / Bun;**280 tests**,CI 绿)。已落地:多跳/su PTY 会话引擎、读/改命令分类器、SQLite 环境注册表、`lanternd` 守护进程 + `lantern` CLI(unix socket RPC)、OS 钥匙串密钥 + socket 能力令牌 + ssh host-key pin、`lantern env init` 交互式接环境向导(RFC-0002)、`lantern watch` 只读实时镜像(RFC-0001)、`lantern swap` 换包闭环(RFC-0003)、`lantern observe` 只读侵入式 Arthas 诊断(RFC-0004)、opencode 加固权限配置 + `env-debugger` agent。`connectSsh2`(真实 ssh)需活动 sshd,以受控 e2e 验证;DB/Redis/Kafka 存储、复现回路、Web 控制台属 Phase 2。设计文档:
...
- [`design/rfc/0004-lantern-observe.md`](./design/rfc/0004-lantern-observe.md) — RFC-0004:`lantern observe` 只读侵入式 Arthas 诊断(已实现)。
